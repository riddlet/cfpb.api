...
#' }
#'
query_aggregates <- function(search_term = NULL, agg_type = 'state', field = NULL,
                             ...
                             zip_code = NULL)
{
  ...
  if (agg_type == 'product')
  {
    ...
    subproducts <- text_res$aggregations$product$product$buckets$product$buckets
    outdat <- dplyr::bind_rows(subproducts)
    outdat$product <- rep(products, sapply(subproducts, nrow))
  ...
  } else if (agg_type == 'issue')
  {
    ...
    subissues <- text_res$aggregations$issue$issue$buckets$issue$buckets
